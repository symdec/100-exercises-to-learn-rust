...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError { error_msg: _ }) => {
            Ticket::new(title, "Description not provided".to_string(), status).unwrap()
        }
        Err(TicketNewError::TitleError { error_msg }) => {
            panic!("{error_msg}");
        }
    }
}
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                error_msg: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                error_msg: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                error_msg: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                error_msg: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
